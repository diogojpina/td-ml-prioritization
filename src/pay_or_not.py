...
for predictor in predictores:
    ...
    predictor['accuracy'] = max(np.nan_to_num(clf.cv_results_['mean_test_accuracy']))
...
def test_method(model, params):
    ...
    print('Precision', precision_score(target_test, target_predicted))
    print('Recall', recall_score(target_test, target_predicted))
    print('F1', f1_score(target_test, target_predicted))
    print("")
...
models_test = [
    { 
        'name': 'Dummy',
        'model': dummy.DummyClassifier(),
        'params': {'strategy': 'stratified', 'random_state': 44, 'constant': 0}
    },
    { 
        'name': 'NB',
        'model': naive_bayes.GaussianNB(),
    ...
    { 
        'name': 'KNN',
        'model': neighbors.KNeighborsClassifier(), 
        'params': {'weights': 'distance', 'p': 1, 'n_neighbors': 2, 'algorithm': 'brute'}
    },     
    { 
        'name': 'LR',
        'model': linear_model.LogisticRegression(), 
        'params': {'solver': 'liblinear', 'penalty': 'l2', 'C': 0.6781266902086813}
    },    
    { 
        'name': 'Linear',
        'model': linear_model.RidgeClassifier(),
        'params': {'solver': 'auto', 'alpha': 21.109816247565362}
    },
    { 
        'name': 'SVM',
        'model': svm.SVC(),
        'params': {'kernel': 'linear', 'C': 2.087256950642817}
    },
    { 
        'name': 'Decision Tree',
        'model': tree.DecisionTreeClassifier(),
        'params': {'splitter': 'best', 'min_weight_fraction_leaf': 0, 'min_samples_split': 2, 'min_samples_leaf': 1, 'max_leaf_nodes': 1000, 'max_features': 'log2', 'max_depth': 40, 'criterion': 'log_loss'}
    },
    { 
        'name': 'Random Forest',
        'model': ensemble.RandomForestClassifier(),
        'params': {'n_estimators': 52, 'min_weight_fraction_leaf': 0, 'min_samples_split': 3, 'min_samples_leaf': 1, 'max_features': 1, 'max_depth': 40, 'criterion': 'gini'}
    },
    { 
        'name': 'XGBoost',
        'model': xgb.XGBClassifier(),
