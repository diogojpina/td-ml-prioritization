...
model = svm.SVC()
params = {
    "C": [uniform.rvs(0.01, 10) for i in range(0, 4)],
...
models_test = [
    { 
        'name': 'Dummy',
        'model': dummy.DummyClassifier(),
        'params': {'strategy': 'uniform', 'constant': 0}
    },
    { 
        'name': 'NB',
        'model': naive_bayes.GaussianNB(),
    ...
    { 
        'name': 'LR',
        'model': linear_model.LogisticRegression(), 
        'params': {'solver': 'newton-cg', 'penalty': 'none', 'C': 48.95042064733014}
    },    
    { 
        'name': 'Linear',
        'model': linear_model.RidgeClassifier(),
        'params': {'solver': 'auto', 'alpha': 85.52997949254991}
    },
    { 
        'name': 'SVM',
        'model': svm.SVC(),
        'params': {'kernel': 'linear', 'C': 5.99941492459917}
    },
    { 
        'name': 'Decision Tree',
        'model': tree.DecisionTreeClassifier(),
        'params': {'splitter': 'random', 'min_weight_fraction_leaf': 0, 'min_samples_split': 2, 'min_samples_leaf': 1, 'max_leaf_nodes': 1000, 'max_features': 'log2', 'max_depth': 40, 'criterion': 'entropy'}
    },
    { 
        'name': 'Random Forest',
        'model': ensemble.RandomForestClassifier(),
        'params': {'n_estimators': 68, 'min_weight_fraction_leaf': 0, 'min_samples_split': 2, 'min_samples_leaf': 1, 'max_features': 1, 'max_depth': 30, 'criterion': 'gini'}
    },
    { 
        'name': 'XGBoost',
        'model': xgb.XGBClassifier(),
        'params': {'subsample': 0.8842602205813299, 'objective': 'reg:squarederror', 'n_estimators': 170, 'max_depth': 30, 'learning_rate': 0.13983536537448038, 'colsample_bytree': 0.22094694129271064, 'booster': 'dart'}
    },
]
